...
class PurchaseDetailsDialog(QtWidgets.QDialog):
    def __init__(self, invoice_id):
        super().__init__()
        self.invoice_id = invoice_id
        self.db = DatabaseManager()

        self.setWindowTitle("تفاصيل فاتورة الشراء")
    ...
    def build_ui(self):
        ...
        self.lbl_title = QtWidgets.QLabel("تفاصيل فاتورة الشراء")
        ...
        self.table.setHorizontalHeaderLabels(
            ["الصنف", "الكمية", "السعر", "الإجمالي", "المورد"]
        )
    ...
    # -----------------------------------------------------------
    def load_data(self):
        data = self.db.get_purchase_items(self.invoice_id)
        ...
        for i, row in enumerate(data):
            ...
            self.table.setItem(i, 4, QtWidgets.QTableWidgetItem(row["supplier"]))

    # -----------------------------------------------------------
    def export_pdf(self):
        invoice = self.db.get_purchase_invoice_header(self.invoice_id)
        items = self.db.get_purchase_items(self.invoice_id)

        Exporter.export_purchase_pdf(invoice, items)
