...
class SalesEntryPage(QtWidgets.QWidget):
    ...
    def build_ui(self):
        ...
        title = QtWidgets.QLabel("🛒 إنشاء فاتورة مبيعات")
        title.setStyleSheet("""
            font-size: 26px;
            font-weight: bold;
            color: #0A3D91;
        ...
        self.customer_combo.addItem("— اختر العميل —", -1)
        ...
        self.table_items.setHorizontalHeaderLabels(["ID", "الصنف", "الكمية", "السعر"])
        ...
        self.table_cart.setHorizontalHeaderLabels(["الصنف", "الكمية", "السعر", "الإجمالي", "—"])
    ...
    def add_item_to_cart(self):
        ...
        price, ok = QtWidgets.QInputDialog.getDouble(self, "السعر", "ادخل السعر:", 0, 0)
        if not ok:
            return
    ...
    def save_invoice(self):
        ...
        if customer_id == -1:
            QtWidgets.QMessageBox.warning(self, "⚠", "اختر العميل.")
            return

        invoice_id = self.db.create_sales_invoice(customer_id, self.cart)

        QtWidgets.QMessageBox.information(self, "✔", f"تم حفظ الفاتورة بنجاح.\nرقم الفاتورة: {invoice_id}")
    ...
    def print_invoice(self):
        ...
        InvoicePrinter().print_sales_invoice(customer, self.cart, total_value)
